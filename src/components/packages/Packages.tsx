...
import { SpatialStackScroll, type StackItem } from "@/components/spatial/SpatialStackScroll";
...
export async function Packages({ lang }: { lang: keyof LocalizedText }) {
  ...
  function renderCard(pkg: Package, i: number) {
    ...
    return (
      <article className="grid gap-8 md:grid-cols-12 md:items-center">
        <div className={cn("relative", reversed ? "md:col-span-5 md:col-start-8" : "md:col-span-5")}>
          ...
          <div aria-hidden className="absolute -inset-3 -z-10 rotate-2 border border-border bg-border/40" />
          <ImageWrapper
            src={pkg.image}
            alt={pkg.name[lang]}
            fill
  ...
  const stackItems: StackItem[] = packages.map((pkg, i) => ({
    key: pkg.slug,
    content: (
      <div className="flex h-full w-full items-center overflow-y-auto rounded-sm border border-border bg-canvas p-6 shadow-[0_24px_64px_-40px_rgba(32,27,24,0.35)] md:p-10">
        {renderCard(pkg, i)}
      </div>
    ),
  }));
  ...
  return (
    <Section id="packages">
      ...
      <div className="mt-10">
        <SpatialStackScroll items={stackItems} staticContent={staticContent} />
      </div>
    </Section>
  );
}
